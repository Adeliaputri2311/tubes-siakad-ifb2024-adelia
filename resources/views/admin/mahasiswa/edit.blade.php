<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col">
            ...
            <p class="text-xs text-slate-400 font-medium mt-0.5">Perbarui informasi berkas mahasiswa serta alokasi dosen wali pembimbing.</p>
        </div>
    </x-slot>

    <div class="max-w-2xl">
        <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-8">
            <form action="{{ route('admin.mahasiswa.update', $mahasiswa->npm) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')

                <div class="space-y-1.5">
                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider">NPM (Kunci Utama - Tidak Dapat Diubah)</label>
                    <div class="relative">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="nama" id="nama" value="{{ old('nama', $mahasiswa->nama) }}" maxlength="50" 
                ...
                <div class="space-y-1.5">
                    ...
                    <select name="nidn" id="nidn" class="block w-full rounded-xl border-slate-200 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50 text-sm px-4 py-3" required>
                        @foreach($dosens as $dsn)
                            <option value="{{ $dsn->nidn }}" {{ old('nidn', $mahasiswa->nidn) == $dsn->nidn ? 'selected' : '' }}>
